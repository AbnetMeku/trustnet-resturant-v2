    ...
    try {
      ...
      const updatedItems = items
        .filter((item) => {
          ...
          if (selectedTable?.is_vip && (item.vip_price == null)) return false;
        ...
        .map((item) => {
          ...
          return {
            ...
            price,
            increment,
            usingVip, // mark if VIP pricing is active
          };
        });
...
>
  ...
  <div className="absolute inset-0 bg-black/40 flex flex-col justify-end p-2 md:p-3 text-white text-xs md:text-sm">
    <h3 className="font-bold truncate flex items-center gap-2">
      {item.name}
      {item.usingVip && (
        <span className="bg-amber-600 text-black text-[10px] font-bold px-2 py-0.5 rounded-md">
          VIP
        </span>
      )}
    </h3>
