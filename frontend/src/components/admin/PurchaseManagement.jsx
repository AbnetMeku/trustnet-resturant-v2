...
export default function PurchaseManagement() {
  ...
  const loadPurchases = async () => {
    try {
      ...
      setPurchases(data.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)));
    } catch {
    ...
    return (
      <div ref={wrapperRef} className="relative">
        <input
          ...
          onChange={(e) => { setQ(e.target.value); setOpen(true); if (selectedItem && e.target.value !== selectedItem.name) onChange(""); }}
        />
        {open && (
          <ul className="absolute z-50 bg-white border rounded mt-1 max-h-48 overflow-auto w-full shadow">
            {filtered.length ? filtered.map((it) => (
              <li key={it.id} className="p-2 hover:bg-gray-100 cursor-pointer" onMouseDown={(ev) => { ev.preventDefault(); onChange(String(it.id)); setOpen(false); setQ(""); }}>
                {it.name}
              </li>
  ...
  return (
    <Card className="p-6 w-full">
      ...
      <Dialog open={showDialog} onOpenChange={(v) => { setShowDialog(v); dialogOpenRef.current = v; }}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingId ? "Edit Purchase" : "Add Purchase"}</DialogTitle></DialogHeader>
          <div className="flex flex-col gap-3">
            ...
            {itemsLoading ? <div className="p-2">Loading items...</div> :
              <SearchableSelect items={menuItems} value={form.menu_item_id} onChange={(val) => setForm({ ...form, menu_item_id: val })} placeholder="Type to search items..." />
            }

            <Input placeholder="Quantity" type="number" min="1" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} />
            <Input placeholder="Unit Price" type="number" min="0" value={form.unit_price} onChange={(e) => setForm({ ...form, unit_price: e.target.value })} />
