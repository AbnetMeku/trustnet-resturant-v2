...
export default function StationLogin() {
  ...
  useEffect(() => {
    ...
    if (token) navigate('/kds'); // auto-redirect if already logged in
  }, [navigate]);
  ...
  const handleSubmit = async (submittedPin = pin) => {
    ...
    setError('');
    try {
      const res = await axios.post('http://localhost:5000/auth/pin/station', { pin: submittedPin });
      loginStation(res.data.access_token); // use updated AuthContext for stations
      navigate('/kds');
    } catch (err) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-4 md:p-8">
      ...
      <div className="relative z-10 bg-white/20 backdrop-blur-md shadow-2xl rounded-xl p-6 md:p-10 w-full max-w-xs md:max-w-sm flex flex-col items-center">
        ...
        <h2 className="text-2xl md:text-3xl font-bold mb-4 text-center text-white">Station Sign In</h2>
